...
def similarity(a, b):
	...
	for i in range(0,M):
	    numerator = numerator + ((a.iloc[i] - meanB)*(b.iloc[i] - meanB))

	denominator = M*np.std(a)*np.std(b)
...
def computeDelta(wt, X, Xi):
    ...
    for i in range(0,len(Xi)):
    	b = Xi.iloc[i]
    	exponential = np.exp(wt*similarity(a, (b.iloc[0:(len(b)-1)])))
    	numerator = numerator + b[-1]*exponential
